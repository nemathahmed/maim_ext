...
const sessionId = "4c44a102-1f6d-4e85-86ea-d049c40f5761"; // crypto.randomUUID(); // per window/session
const PROXY_WS_URL = `ws://34.171.7.176:9223/extension?sessionId=${sessionId}`;
let socket = null;
let sidepanelWindowId = null;
let socketOpen = false;

chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === "sidepanel_window_id") {
    sidepanelWindowId = message.windowId;
    console.log("Set sidepanelWindowId to", sidepanelWindowId);
  }
});
                    ...
                    */
                    if (!payload.sessionId) {
                        ...
                        for (const targetInfo of targetInfos) {
                            if (!targetInfo.tabId) continue;
                            if (targetInfo.url.startsWith("chrome") || targetInfo.url.startsWith("devtools")) continue;
                          
                            let tab;
                            try {
                              tab = await chrome.tabs.get(targetInfo.tabId);
                              console.log("tab nam", tab);
                            } catch (e) {
                              console.warn(`Could not get tab for targetId=${targetInfo.targetId}, skipping`, e);
                              continue;
                            }
                          
                            if (tab.windowId !== sidepanelWindowId) {
                              console.log(`Skipping tabId ${targetInfo.tabId} — not in sidepanel window`);
                              await detach_from_tab(targetInfo.tabId);
                              continue;
                            }
                          
                            await detach_from_tab(targetInfo.tabId); // optional: in case already attached
                            const sessionId = await attach_to_tab(targetInfo.tabId, targetInfo.targetId);
                          
                            const notice_params = { sessionId, targetInfo, waitingForDebugger: false };
                            const notice = { method: "Target.attachedToTarget", params: notice_params };
                            socket.send(JSON.stringify(notice));
                          }
                    } else {
...
chrome.tabs.onCreated.addListener(async (tab) => {
    if (tab.windowId !== sidepanelWindowId) return;
  
    const targets = await chrome.debugger.getTargets();
    const newTarget = targets.find(t => t.tabId === tab.id);
    if (!newTarget) return;
  
    const targetInfo = add_fields_to_target(newTarget);
    const sessionId = await attach_to_tab(tab.id, newTarget.targetId);
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.type === "sidepanel_opened") {
      ...
      sidepanelWindowId = message.windowId;
      console.log("Sidepanel opened in window", sidepanelWindowId);
  
      connectWebSocket(); // ✅ Now it's safe to connect
      socketOpen = true;
    }
  });
